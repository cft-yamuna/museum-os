  ...
  class MockWebSocket extends EventEmitter {
    ...
    send(data: string) {
      ...
      if (msg.method === 'Page.captureScreenshot') {
        const fakeData = Buffer.from('fake-screenshot-data').toString('base64');
        process.nextTick(() => {
...
describe('registerDisplayCommands', () => {
  ...
  it('display:rotate works on windows', async () => {
    mockGetPlatform.mockReturnValue('windows');
    const handler = registry.get('display:rotate')!;
    const result = await handler({ rotation: 'left' });
    expect(result).toEqual({ rotation: 'left' });
  ...
  it('display:rotate throws not-supported on darwin', async () => {
    mockGetPlatform.mockReturnValue('darwin');
    const handler = registry.get('display:rotate')!;
...
describe('registerScreenshotCommands', () => {
  ...
  beforeEach(async () => {
    ...
    // Mock fetch to return CDP targets with a valid page
  ...
  it('captures screenshot via CDP and returns result', async () => {
  ...
  it('throws when CDP is not reachable', async () => {
    ...
    const handler = registry.get('kiosk:screenshot')!;
    await expect(handler()).rejects.toThrow('fetch failed');
  });

  it('throws when no Chrome page found via CDP', async () => {
    ...
    const handler = registry.get('kiosk:screenshot')!;
    await expect(handler()).rejects.toThrow('No Chrome page found');
  });
});
