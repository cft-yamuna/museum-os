...
export class KioskManager {
  ...
  async restart(): Promise<KioskStatus> {
    ...
    if (this.shellMode) {
      // Kill Chrome — shell BAT's infinite loop should relaunch it
      this.killAllChrome();
      // Give shell time to relaunch (check every second, up to 10s)
      for (let i = 0; i < 10; i++) {
        await new Promise((r) => setTimeout(r, 1_000));
        if (this.isChromeRunning()) {
          this.logger.info('Shell mode: Chrome relaunched successfully');
          return this.getStatus();
        }
      }
      // Shell failed to relaunch Chrome — trigger reboot to recover
      ...
      try {
        ...
        this.logger.error('Failed to trigger reboot:', err instanceof Error ? err.message : String(err));
      }
      return { ...this.getStatus(), running: false };
    }
