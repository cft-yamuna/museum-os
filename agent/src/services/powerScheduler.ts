 ...
 *
 * Shutdown flow:
 * 1. If enableLocalCron=true, periodically check if current time matches shutdownCron
...
export class PowerScheduler {
  ...
  private readonly localCronAllowed: boolean;
  private readonly remoteScheduleUpdatesAllowed: boolean;
  ...
  constructor(config: PowerScheduleConfig, logger: Logger, wsClient: WsClient) {
    ...
    this.localCronAllowed = isEnabledEnvFlag(ENABLE_LOCAL_CRON_ENV);
    this.remoteScheduleUpdatesAllowed = isEnabledEnvFlag(ALLOW_REMOTE_SCHEDULE_UPDATES_ENV);
  }

  start(): void {
    if (!this.localCronAllowed) {
      if (this.config.enableLocalCron || this.config.shutdownCron || this.config.startupCron) {
        this.logger.warn(
          `PowerScheduler: local cron config ignored. Set ${ENABLE_LOCAL_CRON_ENV}=true to allow local cron`
        );
      }
      this.logger.info('PowerScheduler: local cron scheduler disabled by policy');
      return;
    }

    if (!this.config.enableLocalCron) {
      this.logger.info('PowerScheduler: local cron scheduler disabled (enableLocalCron=false)');
      return;
    }
   ...
   */
  updateSchedule(newConfig: Partial<PowerScheduleConfig>, source: ScheduleUpdateSource = 'remote'): void {
    const blockedFields: string[] = [];
    const blockCronFromRemote = source === 'remote' && !this.remoteScheduleUpdatesAllowed;

    if (newConfig.enableLocalCron !== undefined) {
      if (blockCronFromRemote) {
        blockedFields.push('enableLocalCron');
      } else {
        this.config.enableLocalCron = newConfig.enableLocalCron;
      }
    }
    if (newConfig.shutdownCron !== undefined) {
      if (blockCronFromRemote) {
        blockedFields.push('shutdownCron');
      } else {
    ...
    if (newConfig.startupCron !== undefined) {
      if (blockCronFromRemote) {
        blockedFields.push('startupCron');
      } else {
    ...
    if (blockedFields.length > 0) {
      this.logger.warn(
        `PowerScheduler: ignored remote schedule fields [${blockedFields.join(', ')}]. Set ${ALLOW_REMOTE_SCHEDULE_UPDATES_ENV}=true to allow`
      );
    }

    // Reconcile interval state when schedule config changes at runtime.
    if (!this.localCronAllowed || !this.config.enableLocalCron || !this.config.shutdownCron) {
      if (this.timer) {
        clearInterval(this.timer);
        this.timer = null;
      }
    ...
    this.logger.info(
      `PowerScheduler schedule updated: localCron=${this.config.enableLocalCron ? 'on' : 'off'}, shutdown="${this.config.shutdownCron}"`
    );
  }
  ...
  private checkSchedule(): void {
    if (!this.localCronAllowed) return;
    if (!this.config.enableLocalCron) return;
