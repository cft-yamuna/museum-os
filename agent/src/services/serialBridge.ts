 ...
 *
 * Also recognises line-delimited presence sensor words from combined boards:
...
`;
  ...
  /** Words that arrive as full lines from a combined presence-sensor board. */
  private static readonly SENSOR_WORDS: Record<string, string> = {
    'Present': 'sensor:present',
    'Clear': 'sensor:clear',
    'Ready.': 'sensor:ready',
  };

  private sensorState: 'present' | 'clear' | 'unknown' = 'unknown';
   ...
   *
   * Also buffers incoming text to detect line-delimited sensor words
   * ("Present", "Clear", "Ready.") from combined sensor boards.
   */
  private processChars(text: string): void {
    for (const char of text) {
      if (char === '\n' || char === '\r') {
        // End of line — check if the buffer matches a sensor word
        const word = this.buffer.trim();
        if (word) {
          const sensorEvent = SerialBridge.SENSOR_WORDS[word];
          if (sensorEvent) {
            // Only emit on state change (deduplicate)
            const newState = word === 'Present' ? 'present' : word === 'Clear' ? 'clear' : this.sensorState;
            if (newState !== this.sensorState || sensorEvent === 'sensor:ready') {
              this.sensorState = newState;
              this.emitEvent(sensorEvent);
            }
          }
        }
        this.buffer = '';
      ...
      // Accumulate into line buffer for sensor word detection
      this.buffer += char;

      // Safety: prevent buffer bloat from garbage data
