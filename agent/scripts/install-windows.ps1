...
$existingKiosk = Get-LocalUser -Name $Username -ErrorAction SilentlyContinue
if (-not $existingKiosk) { net user $Username $KioskPassword /add 2>$null }
else { net user $Username $KioskPassword 2>$null }
net user $Username /expires:never 2>$null
...
Set-ItemProperty -Path $RegPath -Name "DefaultPassword" -Value $KioskPassword
