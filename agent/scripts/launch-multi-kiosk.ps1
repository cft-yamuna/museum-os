# Curato shell multi-screen launcher
...
param(
    ...
    [string]$LogFile = "C:\ProgramData\Curato\logs\shell.log"
)
...
if (-not (Test-Path $MultiConfigPath)) {
    ...
    $fallback = Start-Process -FilePath $BrowserPath -ArgumentList @(
        ...
        "--user-data-dir=C:\ProgramData\Curato\chrome-kiosk",
        $FallbackUrl
    ) -PassThru
...
for ($i = 0; $i -lt $entries.Count; $i++) {
    ...
    $userDataDir = "C:\ProgramData\Curato\chrome-kiosk-screen-$screenSlot"
