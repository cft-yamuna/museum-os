...
# One-shot first-run setup for a fresh clone of Curato (Windows, local-only).
...
if (-not $ready) {
  ...
  Warn "Check logs:  docker compose logs -f curato-app"
  Warn "Skipping the agent upload (it needs the server running)."
  exit 1
}
Ok "Server is up at http://localhost:3401  (login: admin@curato.local / admin123)"
...
try {
  ...
  Warn '  New-NetFirewallRule -DisplayName "Curato Server (3401)" -Direction Inbound -Action Allow -Protocol TCP -LocalPort 3401 -Profile Any'
}
...
if (-not ($haveNode -and $haveNpm -and $haveTar -and $haveCurl)) {
  ...
  Warn "Install them, then run: scripts\auto-deploy-agent.ps1 -Force -Platforms windows -CuratoUrl http://localhost:3401"
} else {
  try {
    # auto-deploy-agent.ps1 lives in this same scripts/ folder.
    & "$PSScriptRoot\auto-deploy-agent.ps1" -Force -Platforms windows -CuratoUrl "http://localhost:3401"
    Ok "Agent package built and uploaded."
  } catch {
    Warn "Agent build/upload failed: $($_.Exception.Message)"
    Warn "You can retry later with: scripts\auto-deploy-agent.ps1 -Force -Platforms windows -CuratoUrl http://localhost:3401"
  }
}
...
if (-not (Test-Path $relayScript)) {
  ...
  try {
    ...
    $startupVbs = Join-Path $startupDir 'curato-update-relay.vbs'
    $vbs = @"
' Curato - update relay launcher (auto-generated by setup-all)
...
Write-Host "  Login    : admin@curato.local / admin123" -ForegroundColor Green
Write-Host "  Logs     : docker compose logs -f curato-app"
