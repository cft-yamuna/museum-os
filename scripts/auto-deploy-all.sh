...
#   */5 * * * * /opt/lightman/App01/scripts/auto-deploy-all.sh >> /var/log/lightman-deploy.log 2>&1

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LIGHTMAN_DIR="${LIGHTMAN_DIR:-$(dirname "$SCRIPT_DIR")}"
LIGHTMAN_URL="${LIGHTMAN_URL:-http://localhost:3401}"
ADMIN_EMAIL="${ADMIN_EMAIL:-admin@museumos.local}"
ADMIN_PASSWORD="${ADMIN_PASSWORD:-admin123}"
PLATFORM="${PLATFORM:-linux}"

STATE_DIR="$LIGHTMAN_DIR/.deploy-state"
mkdir -p "$STATE_DIR"
...
if [ "$NEED_DOCKER_REBUILD" -eq 1 ]; then
  ...
  if [ -f docker-compose.yml ]; then
    docker compose --env-file .env.production up -d --build lightman-app 2>&1 | tail -5
    log "Docker container rebuilt and restarted via docker compose"
  else
    docker build -t lightman-app . 2>&1 | tail -3
    docker stop lightman-app 2>/dev/null || true
    docker rm lightman-app 2>/dev/null || true
