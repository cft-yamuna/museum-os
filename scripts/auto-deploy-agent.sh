...
#   */5 * * * * /opt/lightman/App01/scripts/auto-deploy-agent.sh >> /var/log/lightman-deploy.log 2>&1
#
# Required env vars (or set defaults below):
#   LIGHTMAN_DIR    — path to the App01 repo (default: script's parent dir)
#   LIGHTMAN_URL    — server base URL (default: http://localhost:3401)
...
AGENT_DIR="$LIGHTMAN_DIR/agent"
DISPLAY_DIR="$LIGHTMAN_DIR/display"
DEPLOY_STATE_FILE="$LIGHTMAN_DIR/.agent-deploy-hash"
...
for PLAT in "${PLATFORM_LIST[@]}"; do
  ...
  UPLOAD_RESULT=$(curl -sf "$LIGHTMAN_URL/api/agent/upload" \
