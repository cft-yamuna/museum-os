...
#   */5 * * * * /opt/curato/App01/scripts/auto-deploy-agent.sh >> /var/log/curato-deploy.log 2>&1
#
# Required env vars (or set defaults below):
#   CURATO_DIR    — path to the App01 repo (default: script's parent dir)
#   CURATO_URL    — server base URL (default: http://localhost:3401)
#   ADMIN_EMAIL     - admin login email (default: admin@curato.local)
...
AGENT_DIR="$CURATO_DIR/agent"
DISPLAY_DIR="$CURATO_DIR/display"
DEPLOY_STATE_FILE="$CURATO_DIR/.agent-deploy-hash"
...
for PLAT in "${PLATFORM_LIST[@]}"; do
  ...
  UPLOAD_RESULT=$(curl -sf "$CURATO_URL/api/agent/upload" \
